...
export function layoutFrames(key: string, rows: ArtBox[][], normalized = true): ArtPlacement[][] {
  ...
  return rows.map((row, r) => {
    ...
    return row.map((b, c) => {
      ...
      return { x: Math.round((256 - w) / 2), y: 242 - h, w, h, scale };
    });
  });
}
        ...
        const lift =
          s.panel === 'actions' && r === 3 && c === 2
            ...
            : s.panel === 'gait' && r === 2 && c % 2 === 1
              ? 12
              : 0;
        return { x, y: 242 - h - lift, w, h, scale };
      }),
    );
  });
}
...
export function correctMotionPixels(
  ...
  // Head correction was previously tuned for retired characters and is now intentionally
  // disabled in the current two-character configuration.
  return;
}
